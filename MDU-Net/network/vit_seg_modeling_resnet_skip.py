...
class ResNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.root(x)
        features.append(x)
        # print(f"After root size: {x.size()}")

        x = nn.MaxPool2d(kernel_size=3, stride=2, padding=1)(x)  # Add padding=1 here
        # print(f"After MaxPool2d size: {x.size()}")

        downsample_rates = [4, 8, 16]  # Because we expect the sizes to be 1/4, 1/8, 1/16 of the input size
        for i in range(len(self.body) - 1):
            x = self.body[i](x)
            right_size = in_size // downsample_rates[i]
            # print(f"Expected right_size: {right_size}, actual size: {x.size()}")
