...
from UARNet.UARNet9 import UARNet10_1
...
def train(train_loader, model, optimizer, epoch, best_loss):
    ...
    for i, pack in enumerate(train_loader, start=1):
        for rate in size_rates:
            ...
            # ---- rescale ----
            trainsize1 = int(round(352 * rate / 32) * 32)
            trainsize2 = int(round(352 * rate / 32) * 32)
            if rate != 1:
    ...
    save_path = 'polyp/{}/'.format(opt.train_save)
    os.makedirs(save_path, exist_ok=True)

    if (epoch + 1) % 1 == 0:
        torch.save(model.state_dict(), save_path + 'UCRNet10_1.pth')
        print('[Saving Snapshot:]', save_path + 'UCRNet10_1-%d.pth' % epoch)
...
def test(model, path):
    ...
    for s in ['test']:
        ...
        print("test-length", test_loader.size)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train_save', type=str, default='kUCRNet10_1')
    ...
    for epoch in range(1, opt.epoch + 1):
        train(train_loader, model, optimizer, epoch, best_loss)
